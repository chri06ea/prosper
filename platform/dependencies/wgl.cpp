...
namespace lib
{
    ...
    void init_wgl_function_pointers()
    {
        ...
        if (!RegisterClassExA(&window_class))
            FAIL_WITH_ERROR("failed registering window class");
        ...
        if (window_handle == NULL)
            FAIL_WITH_ERROR("failed creating window");
        ...
        if (!wglMakeCurrent(device_context, 0))
            FAIL_WITH_ERROR("failed wglMakeCurrent");

        if (!wglDeleteContext(rendering_context))
            FAIL_WITH_ERROR("failed wglDeleteContext");

        if (!ReleaseDC(window_handle, device_context))
            FAIL_WITH_ERROR("failed ReleaseDC");

        if (!DestroyWindow(window_handle))
            FAIL_WITH_ERROR("failed DestroyWindow");
    }
    ...
    void init_opengl_rendering_context_with_wgl(void* _device_context)
    {
        ...
        if (!wglChoosePixelFormatARB(device_context, pixel_format_attributes, 0, 1, &pixel_format, &num_formats))
            FAIL_WITH_ERROR("failed wglChoosePixelFormatARB");
        ...
        if (!DescribePixelFormat(device_context, pixel_format, sizeof(pixel_format_desc), &pixel_format_desc))
            FAIL_WITH_ERROR("failed DescribePixelFormat");

        // now set pixel format
        if (!SetPixelFormat(device_context, pixel_format, &pixel_format_desc))
            FAIL_WITH_ERROR("failed SetPixelFormat");
        ...
        if (!opengl_rendering_context)
            FAIL_WITH_ERROR("failed wglCreateContextAttribsARB");
